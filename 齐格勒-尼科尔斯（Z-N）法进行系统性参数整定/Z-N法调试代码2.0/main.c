...
volatile uint32_t Angel_timestamp = 0;  
volatile uint16_t Angel_ForSend = 0; 
...
int main(void)
{
	...
	{	
		...
		AngelPID.Kp = 0.5f + RP_GetValue(1) / 4095.0 * 0.2; 
...
//		LocationPID.Kd =RP_GetValue(3) / 4095.0 * 9 ; 
		...
		if(DataReady == 1 )
		{
			...
			DataReady = 0;
			Serial_Printf("%lu,%d,%d,%.3f\r\n", Angel_timestamp, Angel_ForSend,CENTER_ANGEL,AngelPID.Kp);
